...
async function loadSettingsOverview() {
  ...
  try {
    const [methodsRes, packagesRes, usersRes, trainersRes, schedulesRes] = await Promise.all([
      ...
      http.get<Paginated<unknown>>('/admin/users?per_page=1'),
      http.get<Paginated<unknown>>('/admin/trainers?per_page=1'),
      http.get<Paginated<unknown>>('/admin/schedules?per_page=1'),
    ])
