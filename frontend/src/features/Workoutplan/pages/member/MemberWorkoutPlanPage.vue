...
export default {
  ...
  methods: {
    ...
    async deleteWorkout(id) {
      const accepted = await confirmation.ask({ title: 'Hapus jadwal', message: 'Delete this schedule?', confirmText: 'Terima', cancelText: 'Tolak' });
      if (!accepted) return;
    ...
    async clearAllWorkouts() {
      const accepted = await confirmation.ask({ title: 'Hapus semua jadwal', message: 'Delete your entire workout schedule?', confirmText: 'Terima', cancelText: 'Tolak' });
      if (!accepted) return;
      try {
