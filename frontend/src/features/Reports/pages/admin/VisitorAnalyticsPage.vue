...
import { landingVisitApi } from '@/features/Landing/api/landingVisitApi'
...
import type { LandingVisit } from '@/features/Landing/types/landingVisit'
...
const ANALYTICS_VISIT_SAMPLE_SIZE = 100

onMounted(async () => {
  await run(async () => {
    await store.loadSummary()
    // Backend caps per_page at 100, so keep chart sampling within that limit.
    const res = await landingVisitApi.list({ per_page: ANALYTICS_VISIT_SAMPLE_SIZE })
