  ...
  <div class="modal-card">
    ...
    <form @submit.prevent="onSubmit" style="display: grid; gap: 1rem;">
      ...
      <div v-if="values.session_type === 'offline'" class="form-field">
        ...
        <input v-model="values.location" type="text" class="form-input" placeholder="Gym name or address..." />
      </div>

      <div class="form-field">
        <label class="form-label">Notes <span style="opacity: 0.4; font-weight: 400;">(optional)</span></label>
        <textarea v-model="values.member_notes" rows="2" class="form-input" style="resize: none;" placeholder="Training goals, health conditions, etc..." />
      </div>
      ...
      <div v-if="estimatedPrice > 0" class="panel" style="background: var(--color-cream); display: flex; justify-content: space-between; align-items: center;">
        <div>
          <p class="stat-label" style="font-size: 0.75rem;">Estimated Cost</p>
          <p class="text-muted" style="font-size: 0.75rem;">{{ durationHours }} hour(s) × {{ formatPrice(trainer.price) }}</p>
        </div>
...
const onSubmit = handleSubmit(async (formValues) => {
  try {
    await store.createBooking({
      ...
      location: formValues.location || 'Main Gym',
