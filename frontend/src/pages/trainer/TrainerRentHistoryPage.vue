    ...
    <div class="trainer-report-page">
      ...
      <section class="metrics-row">
        ...
        <div class="metric-box bg-white shadow-sm border border-gray-100">
          <div class="box-top">
            <span class="box-label">Total Transactions</span>
            <div class="icon-ring bg-sky text-sky-dark">
              <span class="material-symbols-outlined">receipt_long</span>
            </div>
          </div>
          <div class="box-bottom">
            <div class="box-val">{{ store.summary?.total_records || 0 }}</div>
          </div>
        </div>
        ...
        <div class="metric-box bg-white shadow-sm border border-gray-100">
          <div class="box-top">
            <span class="box-label">Total Earnings</span>
            <div class="icon-ring bg-purple text-purple-dark">
              <span class="material-symbols-outlined">payments</span>
            </div>
          </div>
          <div class="box-bottom">
            <div class="box-val font-medium">{{ currency(store.summary?.total_trainer_amount || 0) }}</div>
          </div>
        </div>
        ...
        <div class="metric-box bg-white shadow-sm border border-gray-100">
          <div class="box-top">
            <span class="box-label">Pending Disbursement</span>
            <div class="icon-ring bg-orange text-orange-dark">
              <span class="material-symbols-outlined">history</span>
            </div>
          </div>
          <div class="box-bottom">
            <div class="box-val font-medium">{{ currency(store.summary?.pending_amount || 0) }}</div>
          </div>
        </div>
        ...
        <div class="metric-box bg-white shadow-sm border border-gray-100">
          <div class="box-top">
            ...
            <div class="icon-ring bg-green text-green-dark">
              <span class="material-symbols-outlined">done_all</span>
            </div>
          </div>
          <div class="box-bottom">
            <div class="box-val font-medium">{{ currency(store.summary?.disbursed_amount || 0) }}</div>
          </div>
        </div>
      </section>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col mt-6 p-6">
        ...
        <div class="table-responsive-wrapper">
          <SkeletonTable v-if="(initialLoading || store.loading) && store.items.length === 0" :columns="6" :rows="8" :style="shimmerStyle" />
          <table v-else class="riwayat-table">
            ...
            <tbody>
              <tr v-for="row in store.items" :key="row.id">
                ...
                <td>{{ formatDate(row.payment?.payment_date || row.booking?.booking_date) }}</td>
                <td>
                ...
                <td>
                  <strong class="session-name">{{ row.booking?.session_type || 'Personal Training' }}</strong>
                  <span class="session-time-sub">{{ row.booking?.start_time ? row.booking.start_time.substring(0, 5) : '' }}</span>
                </td>
        ...
        <div class="pagination-footer border-t border-gray-100 pt-4 mt-4">
          ...
          <div class="pager-controls">
            ...
            <button
              class="pager-action-btn"
              :disabled="store.page >= store.lastPage || store.loading"
