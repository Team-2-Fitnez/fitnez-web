...
class MemberMembershipController extends Controller
{
    ...
    public function uploadProof(Request $request, Payment $payment)
    {
        if ($payment->user_id !== $request->user()->id) {
            return ApiResponse::error('You are not allowed to upload proof for this payment.', [], 403);
        }
        ...
        $payment->update([
            'payment_proof_path' => $data['payment_proof']->store('payment-proofs', 'public'),
            'payment_status' => 'pending',
        ]);

        return ApiResponse::success('Payment proof uploaded. Please wait for admin verification.', $payment->fresh());
    }

    public function pendingRenewals(Request $request)
    {
        $payments = Payment::query()
            ->with(['user.role', 'user.membershipPackage'])
    ...
    public function confirmRenewal(Request $request, Payment $payment)
    {
        if ($payment->payment_type !== 'membership_renewal') {
            return ApiResponse::error('This payment is not a membership renewal.', [], 422);
        }
        ...
        Notification::query()->create([
            ...
            'title' => 'Membership Renewal Confirmed',
            'body' => 'Your membership renewal payment has been confirmed.',
            'notification_type' => 'membership_renewal',
            'is_read' => false,
        ]);

        return ApiResponse::success('Membership renewal confirmed.', $payment->fresh(['user']));
    }
    ...
    public function rejectRenewal(Request $request, Payment $payment)
    {
        ...
        Notification::query()->create([
            'user_id' => $payment->user_id,
            'title' => 'Membership Renewal Rejected',
            'body' => $data['reason'] ?? 'Your membership renewal payment proof could not be verified.',
            'notification_type' => 'membership_renewal',
            'is_read' => false,
        ]);

        return ApiResponse::success('Membership renewal rejected.', $payment->fresh(['user']));
    }

    public function destroyAccount(Request $request)
    {
        if ($confirmation = RequiresConfirmation::check($request, 'delete_account', $request->user()->email)) {
            return $confirmation;
        }
