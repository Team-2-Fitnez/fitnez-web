...
class MemberPaymentController extends Controller
{
    public function index(Request $request)
    {
        $payments = Payment::query()
            ...
            ->orderByDesc('id')
            ->paginate(QueryLimit::perPage($request, 15, 100));
    ...
    public function simulateCreate(Request $request)
    {
        ...
        if ($response = ActionConfirmation::require($request, 'create demo payment', 'new demo invoice')) {
            return $response;
        }
    ...
    public function simulatePay(Request $request)
    {
        ...
        if ($response = ActionConfirmation::require($request, 'confirm demo payment', 'payment #' . $payment->id)) {
            return $response;
        }
