...
class WorkoutPlanController extends Controller
{
    ...
    public function destroy(Request $request, WorkoutPlan $workout_plan): JsonResponse
    {
        ...
        if ($response = ActionConfirmation::require($request, 'delete workout plan', $workout_plan->name ?? ('workout #' . $workout_plan->id))) {
            return $response;
        }

        $workout_plan->delete();

        return ApiResponse::success('Workout plan deleted.', ['deleted_id' => $workout_plan->id]);
    }

    public function clearAll(Request $request): JsonResponse
    {
        if ($response = ActionConfirmation::require($request, 'delete all workout plans', 'all your workout plans')) {
            return $response;
        }

        $deleted = WorkoutPlan::where('user_id', $request->user()->id)->delete();

        return ApiResponse::success('All workout plans deleted.', ['deleted_count' => $deleted]);
    }
}
