...
use App\Features\Schedule\Requests\Admin\StoreScheduleRequest;
use App\Features\Schedule\Requests\Admin\UpdateScheduleRequest;
...
class ScheduleManagementController extends Controller
{
    ...
    public function destroy(\Illuminate\Http\Request $request, TrainerBooking $schedule)
    {
        if ($response = ActionConfirmation::require($request, 'delete schedule', 'schedule #' . $schedule->id)) {
            return $response;
        }

        $schedule->delete();

        return ApiResponse::success('Schedule deleted.', ['deleted_id' => $schedule->id]);
    }
}
