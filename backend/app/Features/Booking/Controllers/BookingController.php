...
class BookingController extends Controller
{
    ...
    public function uploadPaymentProof(Request $request, TrainerBooking $booking)
    {
        ...
        if (! in_array($booking->status, [TrainerBooking::STATUS_PENDING, TrainerBooking::STATUS_PENDING_PAYMENT], true)) {
        ...
        Notification::create([
            ...
            'title'             => $oldPath ? 'Payment Proof Updated' : 'Payment Proof Submitted',
    ...
    public function autoCompleteExpiredBookings(Request $request)
    {
        $count = TrainerBooking::query()
            ->where('status', TrainerBooking::STATUS_CONFIRMED)
            ->whereDate('end_date', '<', now()->toDateString())
            ->update(['status' => TrainerBooking::STATUS_COMPLETED]);

        return ApiResponse::success('Expired bookings auto-completed.', ['completed_count' => $count]);
    }
