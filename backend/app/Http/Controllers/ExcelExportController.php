...
class ExcelExportController extends Controller
{
    ...
    public function attendance(Request $request)
    {
        ...
        foreach ($rows as $row) {
            $this->writeRow($sheet, $r++, [
                ...
                $row->check_in_time ?? '',
                $row->check_out_time ?? '',
                $row->check_out_time ? 'Selesai' : 'Aktif',
            ]);
        }
    ...
    public function nutritionMonitoring(Request $request)
    {
        ...
        $this->writeRow($sheet, 1, ['Member', 'Email', 'Daily Limit (kkal)', 'Total Konsumsi (kkal)', 'Progress (%)', 'Status']);
        ...
        foreach ($mealPlans as $userId => $plans) {
            ...
            $status = $dailyLimit <= 0 ? 'Belum set limit' : ($pct >= 100 ? 'Melebihi limit' : ($pct >= 80 ? 'Hampir penuh' : 'Normal'));
