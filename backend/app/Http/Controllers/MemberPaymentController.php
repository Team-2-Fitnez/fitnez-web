...
class MemberPaymentController extends Controller
{
    ...
    public function simulatePay(Request $request)
    {
        ...
        $notif = Notification::create([
            ...
            'body' => 'Pembayaran ' . $payment->invoice_number . ' sebesar Rp ' . number_format($payment->amount, 0, ',', '.') . ' telah dikonfirmasi.',
            'notification_type' => 'payment_in',
            'is_read' => false,
        ]);
        event(new NewNotification($notif));

        return ApiResponse::success('Pembayaran demo berhasil! (Simulasi)', $payment);
    }
}
