...
class IncomingRentHistoryController extends Controller
{
    ...
    public function breakdown(Request $request)
    {
        ...
        $rows = TrainerEarning::query()
            ...
            ->selectRaw("
                SUM(CASE WHEN LOWER(trainer_bookings.session_type) LIKE '%mentor%' OR LOWER(trainer_bookings.session_type) LIKE '%personal%' THEN trainer_earnings.trainer_amount ELSE 0 END) as mentoring_income,
                SUM(CASE WHEN LOWER(trainer_bookings.session_type) NOT LIKE '%mentor%' AND LOWER(trainer_bookings.session_type) NOT LIKE '%personal%' THEN trainer_earnings.trainer_amount ELSE 0 END) as session_income,
