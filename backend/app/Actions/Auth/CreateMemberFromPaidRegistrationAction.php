...
class CreateMemberFromPaidRegistrationAction
{
    public function handle(ProspectiveMemberRegistration $registration): User
    {
        ...
        return DB::transaction(function () use ($registration) {
            ...
            $role = Role::query()->firstOrCreate(
                ['name' => 'member'],
                ['description' => 'Fitnez member']
            );

            $start = now();
            $expires = now()->addMonths($registration->package->duration_months);
            ...
            $user = User::query()->create([
                ...
                'membership_package_id' => $registration->package->id,
